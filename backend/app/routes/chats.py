...
async def create_chat(chat_in: ChatCreate):
    ...
    for pid in chat["participants"]:
        user_doc = await database.db["users"].find_one({"_id": ObjectId(pid)})
        if user_doc:
            chat["usernames"].append(user_doc.get("username", "Unknown"))
    return ChatOut(**chat)
...
async def get_my_chats(user: UserOut = Depends(get_current_active_user)):
    chats = await database.db["chats"].find({"participants": user.id}).to_list(1000)
    for chat in chats:
        chat["_id"] = str(chat["_id"])
        chat["usernames"] = []
        for pid in chat["participants"]:
...
async def init_chat(user: UserOut = Depends(get_current_active_user)):
    ...
    if existing_chat:
        ...
        existing_chat["usernames"] = []
        for pid in existing_chat["participants"]:
            user_doc = await database.db["users"].find_one({"_id": ObjectId(pid)})
            if user_doc:
                existing_chat["usernames"].append(user_doc.get("username", "Unknown"))
    ...
    chat_data["usernames"] = [user.username, expert.get("username", "Unknown")]
...
async def create_message(message_in: MessageCreate):
    ...
    message_doc["timestamp"] = datetime.utcnow().isoformat() + "Z"
